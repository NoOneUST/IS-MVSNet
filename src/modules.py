...
def generate_FloatTensor(x):
    return ms.Tensor(x, dtype=mstype.float32)


def process_depth_input(depth_start_override, depth_interval_override, B):
    depth_start = depth_start_override if depth_start_override is None else depth_start_override  # n111 or n1hw
    depth_interval = depth_interval_override if depth_interval_override is None else depth_interval_override  # n111

    depth_interval = ms.Tensor(depth_interval, dtype=mstype.float32) if type(
        depth_interval) == float else depth_interval
    depth_start = ms.Tensor(depth_start, dtype=mstype.float32) if type(
        depth_start) == float else depth_start
    depth_interval = depth_interval.reshape(B, 1, 1, 1) if len(depth_interval.shape) == 2 else depth_interval
    depth_start = depth_start.reshape(B, 1, 1, 1) if len(depth_start.shape) == 2 else depth_start
    return depth_start, depth_interval


def get_depth_values(current_depth, n_depths, depth_interval, inverse_depth=False, k=1):
    """
    ...
    if k != 1:
        if n_depths == 16:
            if k == 4:
                interval_multiplier_list = np.logspace(start=0, stop=int(n_depths / 2 - 1), num=int(n_depths / 2),
                                                       base=1.36699)
        elif n_depths == 8:
            if k == 10:
                interval_multiplier_list = np.logspace(start=0, stop=int(n_depths / 2 - 1), num=int(n_depths / 2),
                                                       base=2.86213)
        interval_multiplier_list[0] = interval_multiplier_list[0] / 2
        B = depth_interval.shape[0]
        min_depth_interval = np.broadcast_to(depth_interval / k, (B, int(n_depths / 2), 1, 1))  # (B,n/2,1,1)
        interval_multiplier_list = np.broadcast_to(interval_multiplier_list.view(1, -1, 1, 1), (B, int(n_depths / 2), 1, 1))  # (B,n/2,1,1)
        interval_list = interval_multiplier_list * min_depth_interval  # (B,n/2,1,1)
        depth_bias_list_half = np.cumsum(interval_list, axis=1)  # (B,n/2,1,1)
        flip = op.ReverseV2(axis=[1])
        concat = op.Concat(axis=1)
        depth_bias_list = concat([flip(-depth_bias_list_half), depth_bias_list_half])  # (B,n,1,1)
        depth_values = depth_bias_list + current_depth
        depth_values = np.clip(depth_values, xmin=1e-7, xmax=None)
    else:
